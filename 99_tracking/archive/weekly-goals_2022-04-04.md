---
type: weekly_tracking
date_start: 2022-04-04
tags:
  - tracking
---

# Goals
2022-04-04
---

## weekly

### Open Grants
- [ ] reach out to contacts for HBCU, MSI, TCU
- [x] letter for DSP re: Chelsea and Bess cost-share time
  - [x] follow-up reply to Ashley
  - [ ] DSP sent?
- [ ] cost justification to Lori for coffee and snacks
- [ ] timeline of environmental scan work
- [x] agenda for advisory group meeting (May 19-20)
  - [ ] discussion items
  - [ ] facilitating breakouts + instructions for facilitating
  - [ ] preparation of intro and reading materials
  - [ ] coordinating virtual/hybrid participation
  - [ ] background summary of advisory group for sharing
  - [ ] easels for discussion
  - [ ] prompts for problems or obstacles encountered dealing with grants - where is the friction?
  - [ ] need for guides to walk people over from Holiday Inn in the mornings
- [ ] review and finish webpage updates
- [x] respond to Ashley re: attending
- [x] respond to Jim and Flo re: room

### FAIR Biomed Teaching
- [x] review content for session 1
- [ ] review content for session 2
  + edit slides
- [ ] review content for session 3

### Misc
- [x] feedback on EDM, suwanee causal analysis
	- [ ] record in reference stat
- [ ] add search stats for metabolomics searches
- [ ] add teaching stats for CURE class, Fall 2021 workshops, Spring 2022, Carpentries TTT
- [ ] faculty meeting minutes for April 01

### Omics codeathon
- [ ] review git teaching materials
- [ ] teach session

### ARCS Senior Director Search Committee
- [x] phone interviews (2/2)
- [x] review applications
- [x] contact Helen Tao to schedule phone interview
- [x] schedule phone interview

### Faculty Mentoring
- [x] personal identity wheel worksheet
- [x] pre-session work email
- [ ] scenario discussion slides
- [ ] identity wheel slides
- [ ] takeaway slides
- [ ] intro slides

### GMS 7877 Repro materials
- [ ] review class timeline
- [ ] review discussion materials

### MLA Data Caucus
- [x] coordinate annual meeting before MLA
- [x] identify chair-elect (nominee?)
- [x] prepare meeting agenda and distribute
- [ ] go to Community Council annual Business meeting (April 19)
- [x] annual report (April 1)

### MLA Talk
- [ ] prepare outline
- [ ] prepare slides
- [ ] 

### HSCL DEI / Values Statement
- [ ] respond to Hannah comments
- [ ] send reminder

## Library HiPerGator/ResVault Sponsored Use Program
- [ ] review program description
- [x] review processes for accounts, including allocations, sunsetting, etc.
- [ ] start spreadsheet for active user information
- [ ] Research Computing slack - get information and prep to share
- [ ] develop internal docs for workflow
  + including need for a UF faculty advisor to sign off on research project
  + standardized checks for IRB
  + check-in frequency with library consultant
- [ ] feedback for Research Computing on website info for pathways to access HPG
- [ ] request changes for intake form (Aida)

### LibGuides
- [ ] redo pubmed searching guide
- [ ] review resources for HOBI
- [ ] review resources for biostats
- [ ] review resources for IDP
- [ ] clean up bookmarks and propagate to reproducibility libguide
- [ ] Research Integrity / RCR libguide

### ARCS webpage
- [ ] update AI page ?
- [ ] info for hipergator student program
- [ ] set up march meeting for hipergator program
- [ ] exit assessment for hipergator program

### Modular, Reproducible Data Analysis for the Rest of Us
- [ ] set up meeting for scoping discussion/chat

## Long-term

### Library Carpentry for Joint Chairs
- [ ] draft plan and distribute for Joint Chairs & UF Carpentries Club input
- [ ] distribute plan to UF Carpentries Club

### Ally Skills
- [ ] Spring message

### ReproducibiliTea
- [ ] develop advertising plan
- [ ] carry out advertising

### DEI team
- [ ] look into trainings on activism or institutional change

### FORCE11 data-ethics WG
- [ ] examples and template language for journal policy

### Milk Metabolomics Searching

### MEE time series paper
- [x] skim paper
- [x] invite reviewers
- [ ] read reviews
- [ ] make decision

### MEE stochastic/stability paper
- [x] skim paper
- [x] invite reviewers
- [ ] read reviews
- [ ] make decision


### T32 Rigor & Reproducibility Seminar Series
- Summer 2022
	- [ ] speaker bio on webpage
	- [ ] confirm date and time
	- [ ] setup libcal event
	- [ ] reminder email
	- [ ] advertising
	- [ ] setup google docs
