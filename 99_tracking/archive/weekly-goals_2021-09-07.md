---
type: weekly_tracking
date_start: 2021-09-07
tags:
  - tracking
---

# Goals
2021-09-07
---

## weekly

### MLA Submissions
- [ ] abstract for contributed paper on reproducibility services
	+ reproducibility as more than just research rigor (and NIH's narrow definition)
	+ empowering patrons vs checking for correctness and following guidelines
	+ area: Innovation & Research Practice
- [ ] immersion session on ebookSOS pricing
	- [ ] reach out to Chris Shaffer (UCSF), Collection Development or Scholarly Comms caucuses
- [ ] ~~immersion session on data curation for hospitals?~~

### Misc
- [ ] orientation for Stacy Wright
- [ ] consultation meeting with Thiago de Andrade
- [ ] MEE review
- [ ] HOBI liaison email (to Matt Gurka and Dominick Lemas?)
- [ ] Biostats liaison email (to Ira Longini?)

### UIC Reproducibility Conference
- [ ] discuss talk topics and timing with Vicky Rampin
- [ ] outline talk content
- [ ] abstract to Abigail Goben

### Faculty minutes
- [x] distribute minutes for 2021-09-10
- [x] update minutes on reference share

### HOBI orientation
- [x] contact Ramzi Salloum
- [x] schedule meeting with Stacy Wright

### Informatics Trainers
- [x] public posting of calendar? (share with Lily Lewis)
- [ ] announcement message for September

### Library Carpentry for Joint Chairs
- [ ] draft plan and distribute for Joint Chairs & UF Carpentries Club input
- [ ] distribute plan to UF Carpentries Club

### Ally Skills
- [ ] email and coordinate Fall meetings

### ReproducibiliTea
- [ ] develop advertising plan
- [ ] carry out advertising

### T32 Rigor & Reproducibility Seminar Series
- [ ] set up google docs
- [ ] finalize details
- [ ] September 24 seminar
- [ ] review invitees for Spring, Summer

### Faculty Mentoring
- [ ] draft response to skills survey demographic question
- [ ] generate learning objectives from individual skills on survey

### Open Grants
- [ ] [TODOs](file:///Users/hye/projects/planning-open-grants/TODO.md)
- [x] document stipend process
- [ ] check student applications
- [ ] student hiring publicizing

### JMLA reproducibility paper
- [ ] schedule meeting
- [ ] final revisions
- [ ] submission

### Library HiPerGator/ResVault Sponsored Use Program
- [x] update application form
- [ ] get application form created
- [ ] update ARCS pages
- [ ] review program description
- [ ] review processes for accounts, including allocations, sunsetting, etc.
- [ ] send out program description
- [ ] schedule meeting for discussion

## Long-term

### LibGuides
- [ ] review content for HOBI
- [ ] edit HOBI libguide
- [ ] review content for biostats
- [ ] separate out biostats libguide
- [ ] clean up bookmarks and propagate to reproducibility libguide
